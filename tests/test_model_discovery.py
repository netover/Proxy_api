...
class TestModelInfo:
    ...
    def test_model_info_validation(self):
        ...
        # Invalid timestamp
        with pytest.raises(ValueError, match="created must be a positive Unix timestamp"):
            ModelInfo(
                id="test",
                created=-1,
                owned_by="test",
...
class TestModelDiscoveryService:
    ...
    async def test_discover_models_rate_limit(self, service, provider_config):
        ...
        mock_response = {
            "object": "list",
            "data": [{"id": "test-model", "object": "model", "created": 1234567890, "owned_by": "test", "permissions": []}]
        }
        
        with patch('aiohttp.ClientSession.get') as mock_get:
            # First two calls return 429, third succeeds
            mock_get.side_effect = [
                AsyncMock(status=429, __aenter__=AsyncMock(return_value=AsyncMock(status=429))),
                AsyncMock(status=429, __aenter__=AsyncMock(return_value=AsyncMock(status=429))),
                AsyncMock(status=200, __aenter__=AsyncMock(return_value=AsyncMock(
                    status=200,
    ...
    async def test_validate_model_success(self, service, provider_config):
        ...
        mock_response = {
            "object": "list",
            "data": [
                {"id": "gpt-4", "object": "model", "created": 1687882411, "owned_by": "openai", "permissions": []},
                {"id": "gpt-3.5-turbo", "object": "model", "created": 1677610602, "owned_by": "openai", "permissions": []}
            ]
        }
    ...
    async def test_validate_model_not_found(self, service, provider_config):
        ...
        mock_response = {
            "object": "list",
            "data": [{"id": "gpt-4", "object": "model", "created": 1687882411, "owned_by": "openai", "permissions": []}]
        }
        ...
        with patch('aiohttp.ClientSession.get') as mock_get:
            ...
            is_valid = await service.validate_model(provider_config, "non-existent-model")
    ...
    async def test_skip_invalid_models(self, service, provider_config):
        ...
        mock_response = {
            "object": "list",
            "data": [
                {"id": "valid-model", "object": "model", "created": 1234567890, "owned_by": "test", "permissions": []},
                {"id": "invalid-model", "object": "invalid", "created": 1234567890, "owned_by": "test", "permissions": []},
                {"id": "another-valid", "object": "model", "created": 1234567890, "owned_by": "test", "permissions": []}
            ]
        }
