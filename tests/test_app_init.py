...
class TestApplicationInitializer:
    ...
    async def test_initialize_success(self):
        ...
        with patch("src.core.app_init.settings") as mock_settings, patch(
            ...
            "src.core.app_init.ApplicationInitializer._setup_signal_handlers"
        ) as mock_setup_signals, patch(
            "src.core.app_init.ApplicationInitializer._initialize_services"
    ...
    async def test_initialize_failure(self):
        ...
        with patch(
            "src.core.app_init.settings", side_effect=Exception("Config error")
        ):
            initializer = ApplicationInitializer()
    ...
    async def test_initialize_with_logger_on_failure(self):
        ...
        with patch("src.core.app_init.settings") as mock_settings, patch(
            ...
            with patch(
                "src.core.app_init.ApplicationInitializer._setup_signal_handlers"
            ) as mock_setup_signals:
                mock_setup_signals.side_effect = Exception(
                    "Signal setup failed"
                )
    ...
    def test_setup_signal_handlers_success(self):
        ...
        with patch("signal.signal") as mock_signal:
            ...
            mock_signal.assert_any_call(
                signal.SIGINT, initializer._signal_handler
            )
            mock_signal.assert_any_call(
                signal.SIGTERM, initializer._signal_handler
            )
    ...
    def test_setup_signal_handlers_failure(self):
        ...
        with patch(
            ...
            initializer.logger.warning.assert_called_once_with(
    ...
    def test_signal_handler(self):
        ...
        initializer.logger.info.assert_called_once_with(
    ...
    async def test_initialize_services(self):
        ...
        with patch(
            "src.core.app_init.ApplicationInitializer._initialize_parallel_execution_components"
            ...
            initializer.logger.info.assert_any_call(
    ...
    async def test_initialize_parallel_execution_components_success(self):
        ...
        with patch(
            "src.core.app_init.circuit_breaker_pool"
        ) as mock_circuit_breaker, patch(
            "src.core.app_init.load_balancer"
        ) as mock_load_balancer, patch(
            "src.core.app_init.provider_discovery"
        ) as mock_provider_discovery:
            ...
            # Verify logging
            initializer.logger.info.assert_any_call(
                "🔄 Initializing parallel execution system..."
            )
            initializer.logger.info.assert_any_call(
                "✅ Provider Discovery Service started"
            )
            initializer.logger.info.assert_any_call(
                "✅ Circuit Breaker Pool adaptation started"
            )
            initializer.logger.info.assert_any_call(
                "✅ Load Balancer monitoring started"
            )
            initializer.logger.info.assert_any_call(
    ...
    async def test_initialize_parallel_execution_components_failure(self):
        ...
        with patch(
            ...
            mock_circuit_breaker.start_adaptation_loop.side_effect = Exception(
            ...
            initializer.logger.error.assert_called_once_with(
                "Failed to initialize parallel execution system: Circuit breaker failed"
            )
    ...
    async def test_shutdown_success(self):
        ...
        with patch(
            "src.core.app_init.ApplicationInitializer._shutdown_parallel_execution_components"
        ) as mock_shutdown_parallel:
            initializer = ApplicationInitializer()
            initializer.logger = Mock()
    ...
    async def test_shutdown_parallel_execution_components_success(self):
        ...
        with patch(
            ...
            "src.core.app_init.parallel_fallback_engine"
        ) as mock_parallel_fallback, patch(
            "src.core.app_init.provider_discovery"
        ) as mock_provider_discovery:
            ...
            initializer.logger.info.assert_any_call(
                "🔄 Shutting down parallel execution system..."
            )
            initializer.logger.info.assert_any_call(
                "✅ Parallel execution system shutdown complete"
            )
    ...
    async def test_shutdown_parallel_execution_components_failure(self):
        ...
        with patch(
            ...
            mock_circuit_breaker.shutdown.side_effect = Exception(
                "Shutdown failed"
            )
            ...
            initializer.logger.error.assert_called_once_with(
                "Error during parallel execution shutdown: Shutdown failed"
            )
...
class TestInitializeAppFunction:
    ...
    async def test_initialize_app_default_config(self):
        ...
        with patch(
            "src.core.app_init.ApplicationInitializer"
...
class TestApplicationInitializerIntegration:
    ...
    async def test_full_initialization_flow(self):
        ...
        with patch("src.core.app_init.settings") as mock_settings, patch(
            "src.core.app_init.setup_logging"
        ) as mock_setup_logging, patch("signal.signal") as mock_signal, patch(
            "src.core.app_init.circuit_breaker_pool"
        ) as mock_circuit_breaker, patch(
            "src.core.app_init.load_balancer"
        ) as mock_load_balancer, patch(
