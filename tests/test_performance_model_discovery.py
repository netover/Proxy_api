...
class TestModelDiscoveryPerformance:
    ...
    def test_load_performance_single_provider(self, discovery_service):
        ...
        with patch('src.providers.openai.OpenAIDiscovery.get_models') as mock_openai:
            ...
            assert metrics["memory_increase_mb"] < 100  # Should use less than 100MB additional memory
            
            print(f"Single provider discovery - Time: {metrics['execution_time']:.2f}s, "
    ...
    def test_load_performance_multiple_providers(self, discovery_service):
        ...
        try:
            ...
            def run_discovery():
                return asyncio.run(
                    discovery_service.discover_all_models({
                        "providers": {p: {"enabled": True, "api_key": "test"} for p in providers}
                    })
                )
            ...
            assert metrics["memory_increase_mb"] < 200  # Should use less than 200MB additional memory
            
            print(f"Multi-provider discovery - Time: {metrics['execution_time']:.2f}s, "
                  f"Memory: {metrics['memory_increase_mb']:.2f}MB")
    ...
    def test_cache_performance_benchmark(self, discovery_service):
        ...
        with patch('src.providers.openai.OpenAIDiscovery.get_models') as mock_openai:
            ...
            print(f"Cache performance - Time: {metrics['execution_time']:.2f}s, "
                  f"Memory: {metrics['memory_increase_mb']:.2f}MB")
    ...
    def test_concurrent_access_performance(self, discovery_service):
        ...
        with patch('src.providers.openai.OpenAIDiscovery.get_models') as mock_openai:
            ...
            print(f"Concurrent access - Time: {metrics['execution_time']:.2f}s, "
                  f"Memory: {metrics['memory_increase_mb']:.2f}MB")
    ...
    def test_memory_usage_with_large_datasets(self, discovery_service):
        ...
        huge_model_list = [
            ModelInfo(
                ...
                input_cost=0.01,
                output_cost=0.02,
                description=f"Description for model {i}" * 10  # Add description to increase size
            )
        ...
        with patch('src.providers.openai.OpenAIDiscovery.get_models') as mock_openai:
            ...
            assert metrics["memory_increase_mb"] < 500  # Less than 500MB for 5000 models
            assert len(metrics["result"]) == 5000
            
            print(f"Large dataset - Models: 5000, Memory: {metrics['memory_increase_mb']:.2f}MB")
    ...
    def test_performance_benchmarks(self, discovery_service):
        ...
        for name, metrics in results.items():
            print(f"{name.upper()}: {metrics['models']} models in {metrics['execution_time']:.2f}s "
...
class TestLoadTesting:
    ...
    def test_api_load_test(self):
        ...
        print(f"Load test - 100 requests in {metrics['execution_time']:.2f}s, "
