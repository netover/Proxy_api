...
class TimeoutBenchmarkResult:
    ...
    def timeout_rate(self) -> float:
        return (self.timeout_requests / self.total_requests * 100) if self.total_requests > 0 else 0
...
async def benchmark_timeout_handling():
    ...
    for scenario in scenarios:
        ...
        if v1_result.timeout_times:
            print(f"    V1 Avg Timeout Time: {v1_result.avg_timeout_time*1000:.2f}ms")
        if v2_result.timeout_times:
            print(f"    V2 Avg Timeout Time: {v2_result.avg_timeout_time*1000:.2f}ms")
        print()
    ...
    overall_v1_timeout_rate = total_v1_timeouts / total_requests * 100 if total_requests > 0 else 0
    overall_v2_timeout_rate = total_v2_timeouts / total_requests * 100 if total_requests > 0 else 0
    ...
    fast_timeout_scenarios = [r for r in results.values() if r["config"]["expected_timeouts"] == 1]
    ...
    if fast_timeout_scenarios:
        ...
        for scenario in fast_timeout_scenarios:
            ...
            v1_accuracy = abs(v1_timeout_time - expected_timeout) / expected_timeout * 100
            v2_accuracy = abs(v2_timeout_time - expected_timeout) / expected_timeout * 100
            ...
            try:
                ...
                async with response:
                    if response.status_code < 400:
                        ...
                        result.timeout_requests += 1  # Treat 5xx as timeouts for this test
