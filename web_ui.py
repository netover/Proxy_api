...
def load_config_once() -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Failed to load or validate config.yaml: {e}", exc_info=True
        )
    ...
    try:
        ...
        while f"provider_name_{i}" in form_data:
            ...
            if provider_data.get("api_key_value"):
                env_updates[provider_data["api_key_env"]] = provider_data[
                    "api_key_value"
                ]
        ...
        flash(
            "Configuration saved successfully! Restart the API server for changes to take effect.",
            "success",
        )
...
def _parse_provider_from_form(
    ...
    try:
        ...
        if not models:
            flash(
                f"No valid models specified for provider {provider_name}",
                "error",
            )
            return None
...
def _parse_server_settings(form_data) -> Optional[Dict[str, str]]:
    ...
    try:
        ...
        if api_key_header and not re.match(
            r"^[A-Za-z0-9_-]+$", api_key_header
        ):
            flash("Invalid API key header format", "error")
            return None

        return {"port": port, "api_key_header": api_key_header or "X-API-Key"}
...
def _atomic_save_env(
    env_updates: Dict[str, str], server_settings: Dict[str, str]
) -> bool:
    ...
    try:
        ...
        if server_settings.get("api_key_header"):
            set_key(
                ENV_PATH,
                "PROXY_API_API_KEY_HEADER",
                server_settings["api_key_header"],
            )
...
def require_api_key():
    ...
    if not any(
        k.strip() == api_key for k in settings.proxy_api_keys if k.strip()
    ):
...
def set_key():
    ...
    try:
        ...
        if (
            not ENV_VAR_PATTERN.match(env_var)
            or env_var not in ALLOWED_ENV_VARS
        ):
            flash(
                f"Invalid or disallowed environment variable: {env_var}",
                "error",
            )
            return redirect(url_for("index"))
...
def api_dashboard_providers():
    ...
    for provider in providers:
        provider["status"] = (
            "enabled" if provider.get("enabled", True) else "disabled"
        )
...
def api_discover_models(provider_index):
    ...
    abort(
        501, "Model discovery is temporarily disabled for performance reasons."
    )
...
def api_validate_model(provider_index, model_id):
    ...
    abort(
        501,
        "Model validation is temporarily disabled for performance reasons.",
    )
