...
from proxy_core import ProviderStatus
...
class ModelManager:
    ...
    async def get_provider_models(
        ...
        try:
            ...
            provider_config = ProviderConfig(
                ...
                api_key=getattr(provider, "api_key", "dummy-key")
                or "dummy-key",
            )
    ...
    async def get_model_details(
        ...
        try:
            ...
            self.logger.error(
                f"Failed to get model details for {provider_name}/{model_id}: {e}"
            )
            sanitized_error = error_handler._sanitize_error_message(str(e))
            raise InvalidRequestError(
                f"Failed to retrieve model details: {sanitized_error}"
            )
    ...
    async def update_model_selection(
        ...
        # Validate model exists for provider (simplified for now)
        # In a real implementation, this would check against actual provider APIs
        model_exists = True  # Mock validation
    ...
    async def refresh_models(
        ...
        try:
            # Clear cache
            cache_cleared = await app_state.cache_manager.clear_provider_cache(
                provider_name
            )
    ...
    def _infer_capabilities(self, model_id: str) -> List[str]:
        ...
        if any(
            x in model_lower
            for x in ["vision", "gpt-4-turbo", "claude-3", "gemini-pro-vision"]
        ):
