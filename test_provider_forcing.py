...
async def test_provider_forcing():
    ...
    try:
        ...
        assert (
            config.providers[0].forced == False
        ), "OpenAI should not be forced"
        assert (
            config.providers[1].enabled == True
        ), "Anthropic should be enabled"
        assert (
            config.providers[1].forced == False
        ), "Anthropic should not be forced"
        ...
        openai_provider = next(
            p for p in config.providers if p.name == "openai_test"
        )
        assert openai_provider.enabled == False, "OpenAI should be disabled"
        assert (
            openai_provider.forced == False
        ), "OpenAI should not be forced when disabled"
        ...
        providers = await provider_factory.initialize_providers(
            config.providers
        )
        assert len(providers) == 2, "Should have 2 active providers"

        # Test model selection with forcing
        model_providers = await provider_factory.get_providers_for_model(
            "gpt-3.5-turbo"
        )
        assert len(model_providers) == 1, "Should return only forced provider"
        assert (
            model_providers[0].name == "openai_test"
        ), "Should return forced OpenAI provider"
        ...
        assert (
            len(model_providers) == 1
        ), "Should return Anthropic for its model"
        assert (
            model_providers[0].name == "anthropic_test"
        ...
        try:
            ...
            print(
                "All tests passed! Provider activation/deactivation and forcing functionality is working correctly."
            )
