...
class MarkdownValidator:
    ...
    def validate_file(self, file_path: Path) -> ValidationResult:
        ...
        all_issues = self._comprehensive_analysis(content, lines, str(file_path))
        ...
        return ValidationResult(str(file_path), is_valid, optimized_issues, len(lines))

    def _comprehensive_analysis(self, content: str, lines: List[str], file_path: str) -> List[ValidationIssue]:
    ...
    def _analyze_structure(self, content: str, lines: List[str], file_path: str) -> List[ValidationIssue]:
    ...
    def _analyze_content(self, content: str, lines: List[str], file_path: str) -> List[ValidationIssue]:
    ...
    def _analyze_formatting(self, content: str, lines: List[str], file_path: str) -> List[ValidationIssue]:
    ...
    def _optimize_issues(self, issues: List[ValidationIssue]) -> List[ValidationIssue]:
        ...
        unique_issues = []
        for issue in issues:
            key = (issue.file_path, issue.line_number, issue.issue_type, issue.message)
            if key not in seen:
                seen.add(key)
        ...
        for (file_path, line_num), line_issues in line_groups.items():
            if len(line_issues) == 1:
                ...
                sorted_issues = sorted(line_issues, key=lambda x: self._get_severity_priority(x.severity))
    ...
    def _validate_headers(self, lines: List[str], file_path: str) -> List[ValidationIssue]:
            ...
            if line.startswith('#'):
                ...
                if not re.match(r'^#{1,6}\s', line):
                    issues.append(ValidationIssue(
                        file_path, i, "header_format",
                        "warning", "Header should have space after #",
                ...
                if i > 1:
                    prev_line = lines[i-2]  # i-2 porque enumerate começa em 1
                    if prev_line.strip() and not prev_line.startswith('#'):
                        issues.append(ValidationIssue(
                            file_path, i, "header_spacing_above",
                            "info", "Header should be surrounded by blank lines [Expected: 1; Actual: 0; Above]",
                            "Add blank line before header", True
                        ))
                ...
                if i < len(lines):
                    ...
                    if next_line.strip() and not next_line.startswith('#'):
                        issues.append(ValidationIssue(
                            file_path, i, "header_spacing_below",
                            "info", "Header should be surrounded by blank lines [Expected: 1; Actual: 0; Below]",
                            "Add blank line after header", True
                        ))

        return issues

    def _validate_code_blocks(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for match in matches:
            ...
            if not language:
                issues.append(ValidationIssue(
                    file_path, start_pos, "code_block_language",
                    "warning", "Code block should specify language",
                    "Add language specifier (e.g., ```python)", True
                ))

            # MD031: Blanks around fences
            # Verifica linha antes do bloco
            if start_pos > 1:
                prev_line = lines[start_pos - 2]  # -2 porque start_pos começa em 1
                if prev_line.strip():
                    issues.append(ValidationIssue(
                        file_path, start_pos, "code_block_spacing_before",
                        "info", "Fenced code blocks should be surrounded by blank lines",
                        "Add blank line before code block", True
                    ))
            ...
            if end_pos < len(lines):
                ...
                if next_line.strip():
                    issues.append(ValidationIssue(
                        file_path, end_pos, "code_block_spacing_after",
                        "info", "Fenced code blocks should be surrounded by blank lines",
                        "Add blank line after code block", True
                    ))
        ...
        if open_blocks % 2 != 0:
            issues.append(ValidationIssue(
                file_path, 1, "code_block_unclosed",
                "error", "Unclosed code block found",
                "Ensure all code blocks are properly closed", True
            ))

        return issues

    def _validate_links(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for match in matches:
            ...
            if not url.strip():
                ...
                issues.append(ValidationIssue(
                    file_path, line_num, "link_empty_url",
                    "error", f"Link '{text}' has empty URL",
                    "Provide valid URL", True
                ))
            ...
            if not text.strip():
                line_num = content[:match.start()].count('\n') + 1
                issues.append(ValidationIssue(
                    file_path, line_num, "link_empty_text",
                    "warning", "Link has empty text",
                    "Provide descriptive link text", True
                ))

        return issues

    def _validate_lists(self, lines: List[str], file_path: str) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if stripped.startswith(('- ', '* ', '+ ')) or re.match(r'^\d+\.\s', stripped):
                ...
                if i > 1:
                    prev_line = lines[i-2]  # i-2 porque enumerate começa em 1
                    if prev_line.strip() and not prev_line.startswith(('#', '-', '*', '+')) and not re.match(r'^\d+\.\s', prev_line.strip()):
                        issues.append(ValidationIssue(
                            file_path, i, "list_spacing",
                            "info", "Lists should be surrounded by blank lines",
                            "Add blank line before list", True
                        ))

                # Verifica linha seguinte (deve ter linha em branco depois)
                if i < len(lines):
                    next_line = lines[i]
                    if next_line.strip() and not next_line.startswith(('#', '-', '*', '+')) and not re.match(r'^\d+\.\s', next_line.strip()):
                        issues.append(ValidationIssue(
                            file_path, i, "list_spacing_after",
                            "info", "Lists should be surrounded by blank lines",
                            "Add blank line after list", True
                        ))

        return issues

    def _validate_tables(self, content: str, file_path: str) -> List[ValidationIssue]:
            ...
            if '|' in line and not line.strip().startswith('|'):
                if not in_table:
                    ...
                    if not re.match(r'^\s*\|[\s\-\|:]+\|\s*$', line):
                        issues.append(ValidationIssue(
                            file_path, i, "table_separator",
                            "error", "Invalid table separator line",
                            "Use |---|---| format for table separator", True
                        ))
    ...
    def _validate_link_fragments(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for match in matches:
            ...
            if '#' in url:
                ...
                if fragment != normalized_fragment:
                    line_num = content[:match.start()].count('\n') + 1
                    issues.append(ValidationIssue(
                        file_path, line_num, "link_fragments",
                        "warning", f"Link fragment '{fragment}' should use ASCII characters",
                        f"Replace with '{normalized_fragment}'", True
                    ))
    ...
    def _normalize_fragment(self, fragment: str) -> str:
        ...
        import unicodedata
        normalized = unicodedata.normalize('NFD', fragment).encode('ascii', 'ignore').decode('ascii')
    ...
    def _validate_blank_lines(self, lines: List[str], file_path: str) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines, 1):
            if not line.strip():
                ...
                if consecutive_blanks > 1:
                    issues.append(ValidationIssue(
                        file_path, i, "multiple_blanks",
                        "info", f"Multiple consecutive blank lines [Expected: 1; Actual: {consecutive_blanks}]",
    ...
    def _validate_bare_urls(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for match in matches:
            ...
            line_num = content[:match.start()].count('\n') + 1
            issues.append(ValidationIssue(
                file_path, line_num, "bare_urls",
                "warning", f"Bare URL used: {url[:50]}...",
                "Format URL as [text](url)", True
            ))

        return issues

    def _validate_emphasis_as_heading(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if (stripped.count('**') >= 2 or stripped.count('__') >= 2) and not stripped.startswith('#'):
                # Verifica se parece ser um cabeçalho (maiúsculas, curto)
                if len(stripped) < 100 and any(word[0].isupper() for word in stripped.split() if word):
                    issues.append(ValidationIssue(
                        file_path, i, "emphasis_as_heading",
                        "warning", "Emphasis used instead of a heading",
                        "Use # for heading instead of emphasis", False
                    ))

        return issues

    def _validate_trailing_newline(self, content: str, file_path: str) -> List[ValidationIssue]:
        """Valida newline no final do arquivo (MD047)"""
        issues = []

        if not content.endswith('\n'):
            issues.append(ValidationIssue(
                file_path, len(content.split('\n')), "trailing_newline",
                "info", "Files should end with a single newline character",
                "Add newline at end of file", True
            ))
        elif content.endswith('\n\n'):
            issues.append(ValidationIssue(
                file_path, len(content.split('\n')), "trailing_newline",
                "info", "Files should end with a single newline character",
                "Remove extra trailing newlines", True
            ))

        return issues

    def _validate_duplicate_headings(self, content: str, file_path: str) -> List[ValidationIssue]:
        ...
        for i, line in enumerate(lines, 1):
            if line.startswith('#'):
                ...
                if heading_text in headings:
                    issues.append(ValidationIssue(
                        file_path, i, "duplicate_heading",
                        "warning", f"Multiple headings with the same content: '{heading_text}'",
                        "Rename one of the headings to make them unique", False
                    ))
                else:
    ...
    def fix_file(self, file_path: Path) -> int:
        ...
        fixes_applied = self._apply_intelligent_fixes(content, result.issues, file_path)
    ...
    def _apply_intelligent_fixes(self, content: str, issues: List[ValidationIssue], file_path: str) -> int:
        ...
        for issue_type, type_issues in issues_by_type.items():
            if issue_type in ["header_spacing_above", "header_spacing_below"]:
                fixes_applied += self._fix_header_spacing_batch(lines, type_issues)
            elif issue_type == "multiple_blanks":
                fixes_applied += self._fix_multiple_blanks_batch(lines, type_issues)
            elif issue_type == "header_format":
                fixes_applied += self._fix_header_format_batch(lines, type_issues)
            elif issue_type == "trailing_newline":
                fixes_applied += self._fix_trailing_newline(lines, type_issues)
            elif issue_type in ["code_block_spacing_before", "code_block_spacing_after"]:
                fixes_applied += self._fix_code_block_spacing_batch(content, lines, type_issues)
            elif issue_type == "code_block_language":
                fixes_applied += self._fix_code_block_language_batch(lines, type_issues)
            elif issue_type == "link_fragments":
                fixes_applied += self._fix_link_fragments_batch(lines, type_issues)
            elif issue_type == "list_spacing" or issue_type == "list_spacing_after":
                fixes_applied += self._fix_list_spacing_batch(lines, type_issues)
    ...
    def _fix_header_spacing_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
        ...
        for issue in issues:
            ...
            if issue.issue_type == "header_spacing_above" and line_idx > 0:
                ...
                if lines[line_idx - 1].strip():  # Só adiciona se não houver linha em branco
                    lines.insert(line_idx, "")
                    fixes_applied += 1
                    processed_lines.add(line_idx)

            elif issue.issue_type == "header_spacing_below" and line_idx < len(lines) - 1:
    ...
    def _fix_multiple_blanks_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
    ...
    def _fix_header_format_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
    ...
    def _fix_trailing_newline(self, lines: List[str], issues: List[ValidationIssue]) -> int:
    ...
    def _fix_code_block_spacing_batch(self, content: str, lines: List[str], issues: List[ValidationIssue]) -> int:
    ...
    def _fix_code_block_language_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
        ...
        for issue in issues:
            ...
            if line_idx < len(lines) and lines[line_idx].strip() == '```':
                ...
                if line_idx + 2 < len(lines) and ('api_keys:' in lines[line_idx+2] or 'debug:' in lines[line_idx+2]):
                    lines[line_idx] = '```yaml'
                    fixes_applied += 1
                # Ou adiciona como bash se tiver comandos
                elif line_idx + 2 < len(lines) and any(cmd in lines[line_idx+2] for cmd in ['python', 'cd', 'git', 'npm']):
                    lines[line_idx] = '```bash'
                    fixes_applied += 1
                else:
    ...
    def _fix_link_fragments_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
            ...
            if line_idx < len(lines):
                ...
                if match:
                    ...
                    if fragment != normalized:
                        new_line = line.replace(f'#{fragment}', f'#{normalized}')
                        if new_line != line:
    ...
    def _fix_list_spacing_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
        ...
        for issue in issues:
            ...
            elif issue.issue_type == "list_spacing_after" and line_idx < len(lines) - 1:
    ...
    def _fix_bare_urls_batch(self, lines: List[str], issues: List[ValidationIssue]) -> int:
            ...
            if line_idx < len(lines):
                ...
                for match in matches:
                    ...
                    if 'github.com' in url:
                        ...
                        text = url.split('/')[-1] if '.' in url.split('/')[-1] else 'Link'
    ...
    def analyze_patterns(self, results: List[ValidationResult]) -> Dict[str, Any]:
        ...
        for result in results:
            if result.issues:
                file_issue_counts.append({
                    ...
                    "fixable_count": sum(1 for i in result.issues if i.can_fix)
                })
    ...
    def apply_bulk_fixes(self, results: List[ValidationResult], patterns: Dict[str, Any]) -> int:
        ...
        for file_info in patterns.get("files_with_most_issues", []):
            ...
            if file_path.exists():
                ...
                total_fixes += fixes
                logger.info(f"Bulk fix: {file_path} - {fixes} corrections applied")
    ...
    def generate_report(self, results: List[ValidationResult], output_format: str = "json") -> str:
        ...
        report = {
            "summary": {
                ...
                "total_fixable": sum(sum(1 for i in r.issues if i.can_fix) for r in results)
            },
    ...
    def _group_issues_by_type(self, results: List[ValidationResult]) -> Dict[str, int]:
        ...
        for file_info in report["files"]:
            ...
            if file_info["issues"]:
                ...
                for issue in file_info["issues"]:
                    ...
                    md.append(f"- {severity_icon} Line {issue['line_number']}: {issue['message']}")
...
def main():
    parser = argparse.ArgumentParser(description="Markdown Validator and Converter")
    parser.add_argument("path", help="Path to file or directory to validate")
    parser.add_argument("--validate", action="store_true", help="Validate markdown files")
    parser.add_argument("--fix", action="store_true", help="Auto-fix issues")
    parser.add_argument("--bulk-fix", action="store_true", help="Apply bulk fixes based on patterns")
    parser.add_argument("--report", action="store_true", help="Generate validation report")
    parser.add_argument("--output", help="Output file for report")
    parser.add_argument("--format", choices=["json", "markdown"], default="json", help="Report format")
    parser.add_argument("--verbose", action="store_true", help="Verbose output")
    parser.add_argument("--analyze", action="store_true", help="Analyze patterns in validation results")
    ...
    if args.validate or args.report or args.analyze or args.bulk_fix:
        logger.info("[ANALYSIS] Phase 1: Validating all files for comprehensive analysis...")
        for file_path in markdown_files:
            result = validator.validate_file(file_path)
            results.append(result)

            if args.validate:
                status = "[OK] Valid" if result.is_valid else f"[ERROR] {len(result.issues)} issues"
                logger.info(f"{file_path}: {status}")

    # Análise de padrões se solicitada
    patterns = {}
    if args.analyze and results:
        logger.info("[PATTERN] Phase 2: Analyzing patterns across all files...")
        ...
        if patterns.get("files_with_most_issues"):
            ...
            for file_info in patterns["files_with_most_issues"][:3]:
                print(f"  - {file_info['file']}: {file_info['issue_count']} issues ({file_info['fixable_count']} fixable)")
    ...
    if args.bulk_fix and patterns:
        ...
        logger.info(f"[SUCCESS] Bulk fixes completed: {bulk_fixes} total corrections applied")
    ...
    # Correções individuais se solicitadas
        ...
        for file_path in markdown_files:
            ...
            if fixes > 0:
                ...
                total_fixes += fixes
        logger.info(f"[SUCCESS] Individual fixes completed: {total_fixes} total corrections applied")
