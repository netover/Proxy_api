...
from .registry import ProviderConfig, registry as global_registry, ProviderRegistry
...
class DynamicProviderLoader:
    ...
    def _load_builtin_transformers(self) -> Dict[str, Callable]:
        ...
        transformers = {}
        try:
            from .wrappers import AnthropicTransformer, GoogleTransformer, CohereTransformer
            ...
            logger.warning("Could not import transformers from .wrappers. This is expected if the file doesn't exist yet.")
        return transformers
    ...
    def load_provider_wrapper(self, provider_name: str) -> Any:
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Using GenericOpenAIWrapper for provider '{provider_name}'")
            except ImportError:
                logger.error("GenericOpenAIWrapper not found. Cannot fall back.")
                raise ProviderNotFoundError(f"Wrapper for provider '{provider_name}' not found and fallback failed.")
    ...
    def get_client(self, model_name: str) -> Any:
        ...
        if not config:
            raise ProviderNotFoundError(f"Model '{model_name}' não encontrado no registry")
        ...
        if config.payload_transformer:
            if config.payload_transformer in self.transformers:
                client_instance.set_transformer(self.transformers[config.payload_transformer])
                logger.info(f"Applied transformer '{config.payload_transformer}' to provider '{config.provider}'")
            else:
                logger.warning(f"Transformer '{config.payload_transformer}' not found for provider '{config.provider}'")
