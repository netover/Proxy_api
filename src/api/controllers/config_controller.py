    ...
    try:
        ...
        if hasattr(request.app.state, 'config_mtime'):
            ...
            if config_manager.config_path.exists():
                request.app.state.config_mtime = os.path.getmtime(config_manager.config_path)
        ...
        reload_time = (time.time() - start_time) * 1000
        logger.error("Configuration reload failed", error=str(e), reload_time_ms=reload_time)
...
async def get_config_status(request: Request) -> ConfigStatusResponse:
    ...
    try:
        ...
        file_watching_enabled = hasattr(config_loader, 'observer') and config_loader.observer is not None
