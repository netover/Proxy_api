...
class RequestRouter:
    ...
    async def route_request(
        ...
        for i, provider in enumerate(providers):
            ...
            try:
                ...
                req_dict = (
                    request_data.dict(exclude_unset=True)
                ...
                logger.info(
                    f"Provider {provider.name} doesn't support {operation}, trying next"
                )
                continue
                ...
                logger.error(
                    f"Provider {provider.name} failed for {operation}",
                    error=str(e),
                ...
                if any(
                    ...
                    logger.warning(
                        "Truncating context for streaming request and retrying with next provider."
                    )
        ...
        background_tasks.add_task(
            self._record_failure_metrics, request_id, operation, attempt_info
        )
    ...
    async def _record_failure_metrics(
        self, request_id: str, operation: str, attempt_info: List[Dict]
    ) -> None:
