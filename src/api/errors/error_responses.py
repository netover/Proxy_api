...
def create_error_response_from_exception(exc: Exception,
    ...
    exception_mapping = {
        ...
        UnsupportedOperationError: lambda msg, **kwargs: InternalServerErrorResponse(msg, **kwargs),
        ConfigurationError: InternalServerErrorResponse,
        StreamingError: InternalServerErrorResponse,
        TimeoutError: lambda msg, **kwargs: ServiceUnavailableErrorResponse(msg, **kwargs),
        PayloadTooLargeError: PayloadTooLargeErrorResponse,
        ConcurrentRequestError: lambda msg, **kwargs: RateLimitErrorResponse(msg, **kwargs),
    ...
    if hasattr(exc, 'field'):
        response = response_class(str(exc), field=getattr(exc, 'field', None), request_id=request_id)
    elif hasattr(exc, 'resource'):
        response = response_class(str(exc), resource=getattr(exc, 'resource', None),
                                resource_id=getattr(exc, 'resource_id', None), request_id=request_id)
    elif hasattr(exc, 'retry_after'):
        response = response_class(str(exc), retry_after=getattr(exc, 'retry_after', None), request_id=request_id)
    else:
...
def not_found(resource: str, resource_id: str = None, request_id: str = None) -> NotFoundErrorResponse:
    """Create not found error response."""
    return NotFoundErrorResponse(resource, resource_id, request_id=request_id)

def unauthorized(message: str = "Authentication required", request_id: str = None) -> AuthenticationErrorResponse:
    """Create unauthorized error response."""
    return AuthenticationErrorResponse(message, request_id=request_id)

def forbidden(message: str = "Access denied", request_id: str = None) -> AuthorizationErrorResponse:
    """Create forbidden error response."""
    return AuthorizationErrorResponse(message, request_id=request_id)

def rate_limited(retry_after: int = None, request_id: str = None) -> RateLimitErrorResponse:
    """Create rate limit error response."""
    return RateLimitErrorResponse("Rate limit exceeded", retry_after=retry_after, request_id=request_id)

def service_unavailable(reason: str = None, retry_after: int = None, request_id: str = None) -> ServiceUnavailableErrorResponse:
    """Create service unavailable error response."""
    message = "Service temporarily unavailable"
    if reason:
        message += f": {reason}"
    return ServiceUnavailableErrorResponse(message, retry_after=retry_after, request_id=request_id)

def internal_error(message: str = "Internal server error", request_id: str = None) -> InternalServerErrorResponse:
