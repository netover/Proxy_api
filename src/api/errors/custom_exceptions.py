...
class APIException(Exception):
    ...
    def __init__(self, message: str, status_code: int = 500, code: str = None, details: dict = None):
...
class ProviderUnavailableError(APIException):
    ...
    def __init__(self, provider_name: str, reason: str = None, retry_after: int = None, **kwargs):
        message = f"Provider '{provider_name}' is currently unavailable"
        if reason:
            message += f": {reason}"
        super().__init__(message, status_code=503, code="provider_unavailable", **kwargs)
...
class QuotaExceededError(APIException):
    ...
    def __init__(self, quota_type: str, limit: int = None, reset_time: str = None, **kwargs):
...
class UnsupportedOperationError(APIException):
    ...
    def __init__(self, operation: str, model: str = None, provider: str = None, **kwargs):
        message = f"Operation '{operation}' is not supported"
        if model:
            message += f" for model '{model}'"
        if provider:
            message += f" by provider '{provider}'"
        super().__init__(message, status_code=501, code="unsupported_operation", **kwargs)
...
class ConcurrentRequestError(APIException):
    ...
    def __init__(self, current_requests: int, max_concurrent: int, **kwargs):
        ...
        super().__init__(message, status_code=429, code="concurrent_request_limit", **kwargs)
