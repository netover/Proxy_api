...
class RequestValidator:
    ...
    async def validate_chat_completion_request(
        ...
        try:
            ...
            if not messages:
                raise InvalidRequestError(
                    "Messages array is required", param="messages"
                )
    ...
    async def validate_embedding_request(
        ...
        try:
            ...
            if isinstance(input_text, list):
                ...
                if len(input_text) == 0:
                    raise InvalidRequestError(
                        "Input array cannot be empty", param="input"
                    )
                ...
                for i, text in enumerate(input_text):
                    if not isinstance(text, str):
                        raise InvalidRequestError(
                            f"Input {i} must be a string", param=f"input[{i}]"
                        )
                ...
                if not isinstance(input_text, str):
                    raise InvalidRequestError(
                        "Input must be a string or array", param="input"
                    )
