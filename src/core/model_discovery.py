...
class ModelDiscoveryService:
    ...
    async def discover_models(self, provider_config: ProviderConfig) -> List[ModelInfo]:
        ...
        """
        logger.info(f"Discovering models from provider: {provider_config.name}")
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                logger.info(
                    f"Discovered {len(models)} models from {provider_config.name}"
                )
                return models
    ...
    async def validate_model(
        ...
        the provider. It's more efficient than discovering all models when
        ...
        try:
            ...
            if exists:
                logger.info(f"Model '{model_id}' is valid")
            else:
                logger.warning(
