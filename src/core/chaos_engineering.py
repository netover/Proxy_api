...
class ChaosMonkey:
    ...
    def configure(self, chaos_config: Dict[str, Any]) -> None:
        ...
        if self.enabled:
            ...
            self.logger.info(
                f"Chaos engineering configured with {len(self.active_faults)} faults"
            )
    ...
    async def inject_fault(
        self, fault_config: FaultConfig, operation: str = None
    ) -> Optional[Exception]:
        ...
        try:
            with TracedSpan(
                ...
                if fault_config.fault_type == FaultType.DELAY:
                    ...
                    span.set_attribute(
                        "fault.error_code", fault_config.error_code or 500
                    )
                    raise error
