...
class RetryHistory:
    ...
    def get_success_rate(self, window_size: int = 20) -> float:
        ...
        successes = sum(
            1
            for _ in range(self.success_count)
            if _ >= len(recent_attempts) - window_size
        )
...
class RetryStrategy(ABC):
    ...
    async def execute_with_retry(self, func: Callable, *args, **kwargs):
        ...
        )  # Get general config for max_attempts
...
class ExponentialBackoffStrategy(RetryStrategy):
    ...
    async def should_retry(self, error: Exception, attempt: int) -> bool:
        ...
        if error_type in [ErrorType.CONNECTION, ErrorType.TIMEOUT]:
            ...
            connection_max = effective_config.get(
                "connection_max_attempts", min(2, max_attempts)
            )
        ...
        if error_type == ErrorType.RATE_LIMIT:
            if hasattr(error, "retry_after") and error.retry_after:
                ...
                if retry_after:
                    ...
                    )  # Start at 5 seconds for rate limits
            else:
                base_delay = max(effective_config["base_delay"] * 2, 5.0)
        else:
...
class ImmediateRetryStrategy(RetryStrategy):
    ...
    async def should_retry(self, error: Exception, attempt: int) -> bool:
        ...
        if error_type == ErrorType.SERVER_ERROR:
            if (
                ...
                and self.immediate_retry_count < self.max_immediate_retries
            ):
                self.immediate_retry_count += 1
                return True
    ...
    async def get_delay(self, error: Exception, attempt: int) -> float:
        ...
        if self.immediate_retry_count <= self.max_immediate_retries and (
            ...
            or (
                error_type in [ErrorType.SERVER_ERROR, ErrorType.UNKNOWN]
                and self._is_transient_error(error)
            )
        ):
            ...
            delay = effective_config["base_delay"] * (
                effective_config["backoff_factor"] ** attempt
            )
...
class AdaptiveRetryStrategy(RetryStrategy):
    ...
    async def should_retry(self, error: Exception, attempt: int) -> bool:
        ...
        if error_type in [ErrorType.CONNECTION, ErrorType.TIMEOUT]:
            ...
            effective_config = self.get_effective_config(error_type)
            return weighted_success_rate > success_threshold and attempt < min(
                4, effective_config["max_attempts"]
            )
    ...
    async def get_delay(self, error: Exception, attempt: int) -> float:
        ...
        if error_type == ErrorType.RATE_LIMIT:
            ...
            base_delay = (
                effective_config["base_delay"] * 0.5
            )  # Shorter for connection issues
        elif error_type == ErrorType.SERVER_ERROR:
            base_delay = (
                effective_config["base_delay"] * 1.2
            )  # Slightly longer for server errors
        else:
            base_delay = effective_config["base_delay"]
