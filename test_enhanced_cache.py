...
async def test_enhanced_cache():
    ...
    try:
        ...
        success = await cache.set("models:openai", test_models, category=CacheCategory.MODELS)
        print(f"✓ Set models: {success}")

        retrieved = await cache.get("models:openai", category=CacheCategory.MODELS)
        print(f"✓ Retrieved models: {len(retrieved) if retrieved else 0} items")

        # Test responses category
        test_response = {"result": "test response", "status": "success"}
        success = await cache.set("response:123", test_response, category=CacheCategory.RESPONSES)
        print(f"✓ Set response: {success}")

        # Test summaries category
        test_summary = {"summary": "test summary", "confidence": 0.95}
        success = await cache.set("summary:456", test_summary, category=CacheCategory.SUMMARIES)
        print(f"✓ Set summary: {success}")

        # Test metrics category
        test_metrics = {"hit_rate": 0.85, "response_time": 150}
        success = await cache.set("metrics:api", test_metrics, category=CacheCategory.METRICS)
        ...
        print(f"Models tier: {cache._get_tier_for_category(CacheCategory.MODELS).value}")
        print(f"Responses tier: {cache._get_tier_for_category(CacheCategory.RESPONSES).value}")
        print(f"Summaries tier: {cache._get_tier_for_category(CacheCategory.SUMMARIES).value}")
        print(f"Analytics tier: {cache._get_tier_for_category(CacheCategory.ANALYTICS).value}")

        # Test backward compatibility
        print("\n--- Testing Backward Compatibility ---")

        # Test legacy methods
        success = await cache.set_models("anthropic", "https://api.anthropic.com", [{"id": "claude-2"}])
        print(f"✓ Legacy set_models: {success}")

        models = await cache.get_models("anthropic", "https://api.anthropic.com")
        print(f"✓ Legacy get_models: {len(models) if models else 0} items")

        success = await cache.set_response("legacy:resp:789", {"legacy": "response"})
