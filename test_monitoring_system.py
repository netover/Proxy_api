...
async def test_basic_metrics_collection():
    ...
    metrics_collector.record_request(
        "test_provider", True, 0.5, tokens=100, model_name="test-model"
    )
    metrics_collector.record_request(
        "test_provider",
        False,
    ...
    metrics_collector.record_request(
        "another_provider", True, 0.8, tokens=50, model_name="another-model"
    )
...
async def test_alert_system():
    ...
    for i in range(20):
        metrics_collector.record_request(
            "test_provider", False, 0.5, error_type="test_error"
        )
    ...
    triggered_alerts = [
        a for a in active_alerts if a["rule_name"] == "test_high_error_rate"
    ]
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ Health check successful - Status: {health_data['status']}"
                )
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ Providers tracked: {len(metrics_data.get('providers', {}))}"
                )
                return True
            else:
                print(
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ Active alerts: {len(alerts_data.get('alerts', []))}"
                )
                ...
                if response.status_code == 200:
                    ...
                    print(
                        f"✅ Alert rules: {len(rules_data.get('rules', []))}"
                    )
                    return True
                else:
                    print(
                        f"❌ Alert rules endpoint failed with status {response.status_code}"
                    )
                    return False
            else:
                print(
                    f"❌ Alerts endpoint failed with status {response.status_code}"
                )
                return False
    except Exception as e:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"❌ Monitoring dashboard failed with status {response.status_code}"
                )
                return False
    except Exception as e:
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ Metrics data length: {len(prometheus_data)} characters"
                )
                ...
                print(
                    f"✅ Found {found_metrics}/{len(expected_metrics)} expected metrics"
                )
                return found_metrics > 0
            else:
                print(
                    f"❌ Prometheus metrics failed with status {response.status_code}"
                )
                return False
    except Exception as e:
...
async def main():
    ...
    try:
        async with httpx.AsyncClient(timeout=5.0) as client:
            ...
            if response.status_code != 200:
                print(
                    "❌ Server doesn't appear to be running on localhost:8000"
                )
