...
class TestModelEndpoints:
    ...
    def setup_mocks(self, app, mock_app_state, mock_provider_factory, 
                   mock_model_discovery, mock_config_manager, mock_cache_manager):
    ...
    @pytest.mark.asyncio
    async def test_list_provider_models_success(self, client, app, mock_app_state, 
                                              mock_provider_factory, mock_model_discovery):
        """Test successful listing of models for a provider."""
        # Setup mocks
        self.setup_mocks(app, mock_app_state, mock_provider_factory, 
                        mock_model_discovery, MagicMock(), MagicMock())
    ...
    @pytest.mark.asyncio
    async def test_list_provider_models_provider_not_found(self, client, app, 
        ...
        # Make request
        response = client.get("/v1/providers/nonexistent/models", 
    ...
    @pytest.mark.asyncio
    async def test_get_model_details_success(self, client, app, mock_app_state,
                                           mock_provider_factory, mock_model_discovery):
        ...
        # Mock model
        mock_model = ModelInfo(id="claude-3-opus-20240229", created=1709251200, owned_by="anthropic")
        mock_model_discovery.get_model_info.return_value = mock_model
        
        # Make request
        response = client.get("/v1/providers/anthropic/models/claude-3-opus-20240229",
                            headers={"Authorization": "Bearer test-key"})
    ...
    @pytest.mark.asyncio
    async def test_get_model_details_not_found(self, client, app, mock_app_state,
                                             mock_provider_factory, mock_model_discovery):
        ...
        # Make request
        response = client.get("/v1/providers/openai/models/nonexistent-model",
                            headers={"Authorization": "Bearer test-key"})
    ...
    @pytest.mark.asyncio
    async def test_update_model_selection_success(self, client, app, mock_app_state,
                                                mock_provider_factory, mock_model_discovery,
                                                mock_config_manager):
        """Test successful model selection update."""
        # Setup mocks
        self.setup_mocks(app, mock_app_state, mock_provider_factory, 
                        mock_model_discovery, mock_config_manager, MagicMock())
        ...
        response = client.put("/v1/providers/openai/model_selection",
                            json=selection_data,
                            headers={"Authorization": "Bearer test-key"})
    ...
    @pytest.mark.asyncio
    async def test_update_model_selection_invalid_model(self, client, app, 
                                                      mock_app_state, mock_provider_factory,
                                                      mock_model_discovery, mock_config_manager):
    ...
    @pytest.mark.asyncio
    async def test_refresh_models_success(self, client, app, mock_app_state,
                                        mock_provider_factory, mock_model_discovery,
                                        mock_cache_manager):
        """Test successful model cache refresh."""
        # Setup mocks
        self.setup_mocks(app, mock_app_state, mock_provider_factory, 
                        mock_model_discovery, MagicMock(), mock_cache_manager)
        ...
        # Make request
        response = client.post("/v1/providers/openai/models/refresh",
                             headers={"Authorization": "Bearer test-key"})
    ...
    @pytest.mark.asyncio
    async def test_refresh_models_provider_not_found(self, client, app, 
                                                   mock_provider_factory):
        ...
        # Make request
        response = client.post("/v1/providers/nonexistent/models/refresh",
                             headers={"Authorization": "Bearer test-key"})
    ...
    @pytest.mark.asyncio
    async def test_rate_limiting(self, client, app, mock_app_state,
                               mock_provider_factory, mock_model_discovery):
    ...
    @pytest.mark.asyncio
    async def test_error_handling(self, client, app, mock_app_state,
                                mock_provider_factory):
        """Test error handling for various scenarios."""
        # Setup mocks to raise exception
        app.state.app_state = mock_app_state
        mock_app_state.provider_factory = mock_provider_factory
        mock_provider_factory.get_all_provider_info.side_effect = Exception("Database error")
