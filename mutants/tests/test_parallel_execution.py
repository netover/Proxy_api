...
from src.core.circuit_breaker_pool import circuit_breaker_pool, AdaptiveTimeoutConfig, TimeoutStrategy
from src.core.load_balancer import load_balancer, LoadBalancingStrategy
from src.core.provider_factory import BaseProvider, ProviderStatus, provider_factory


class MockProvider(BaseProvider):
    """Mock provider for testing"""

    def __init__(self, name: str, response_time: float = 0.1, should_fail: bool = False):
...
class TestParallelExecutionIntegration:
    ...
    def setup_method(self):
        ...
        self.providers = {
            ...
            "medium_provider": MockProvider("medium_provider", response_time=0.15),
            "slow_provider": MockProvider("slow_provider", response_time=0.30),
            "failing_provider": MockProvider("failing_provider", should_fail=True)
        }
    ...
    async def test_first_success_execution(self):
        ...
        with mock.patch.object(provider_discovery, 'get_healthy_providers_for_model',
            ...
            called_providers = [p for p in self.providers.values() if p.call_count > 0]
    ...
    async def test_all_providers_fail(self):
        ...
        with mock.patch('src.core.parallel_fallback.provider_factory') as mock_factory:
            mock_factory.get_provider = AsyncMock(side_effect=lambda name: failing_providers.get(name))
            mock_factory.get_all_provider_info = AsyncMock(return_value=[
                type('ProviderInfo', (), {
                    'name': name,
                    'status': ProviderStatus.HEALTHY,
    ...
    async def test_provider_discovery_health_monitoring(self):
        ...
        await provider_discovery.record_request_result("test_provider", True, 100.0)
        await provider_discovery.record_request_result("test_provider", False, 200.0)
        await provider_discovery.record_request_result("test_provider", True, 50.0)
        ...
        assert health.name == "GOOD"  # Should have good health with 2/3 success rate
    ...
    async def test_circuit_breaker_pool_adaptive_timeout(self):
        ...
        breaker = await circuit_breaker_pool.get_provider_breaker(provider_name, config)
        ...
        for _ in range(10):
            ...
            await circuit_breaker_pool._record_execution(provider_name, True, latency)
    ...
    async def test_load_balancer_performance_tracking(self):
        ...
        # Record completion
        await load_balancer.record_request_complete(provider_name, request_id, True, 150.0)
    ...
    async def test_end_to_end_parallel_execution_with_realism(self):
        ...
        realistic_providers = {
            ...
            "anthropic_slow": MockProvider("anthropic_slow", response_time=0.8),
            "perplexity_medium": MockProvider("perplexity_medium", response_time=0.4),
        }

        with mock.patch('src.core.parallel_fallback.provider_factory') as mock_factory:
            mock_factory.get_provider = AsyncMock(side_effect=lambda name: realistic_providers.get(name))
            mock_factory.get_all_provider_info = AsyncMock(return_value=[
                type('ProviderInfo', (), {
                    'name': name,
                    'status': ProviderStatus.HEALTHY,
            ...
            with mock.patch.object(provider_discovery, 'get_healthy_providers_for_model',
                ...
                winner_providers = [r.provider_name for r in successful_results]
                assert all("openai_fast" in provider for provider in winner_providers)

                # Verify performance improvement
                total_execution_time = sum(r.latency_ms for r in successful_results) / 1000
                avg_execution_time = total_execution_time / len(successful_results)

                # With parallel execution, average should be much better than slowest provider
                assert avg_execution_time < 1.0  # Much faster than 0.8s slowest provider
        ...
        with mock.patch.object(provider_discovery, 'get_healthy_providers_for_model',
            ...
            with mock.patch('src.core.parallel_fallback.provider_factory') as mock_factory:
                mock_providers = {
                    ...
                    "perf_test_2": MockProvider("perf_test_2", response_time=0.2)
                }
                mock_factory.get_provider = AsyncMock(side_effect=lambda name: mock_providers.get(name))
