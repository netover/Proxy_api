...
class TestModelDiscoveryExternalCalls:
    ...
    @pytest.mark.asyncio
    async def test_successful_model_discovery(self, discovery_service, provider_config, mock_model_data, mock_cache):
        """Test successful model discovery from external API"""
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_cache_hit(self, discovery_service, provider_config, mock_cache):
        """Test model discovery cache hit"""
        cached_models = [
            ModelInfo(id="gpt-4", object="model", created=1687882411, owned_by="openai"),
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_authentication_error(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_forbidden_error(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_rate_limit_with_retry(self, discovery_service, provider_config, mock_model_data, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_rate_limit_exhausted(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_network_error_with_retry(self, discovery_service, provider_config, mock_model_data, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_network_error_exhausted(self, discovery_service, provider_config, mock_cache):
        ...
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                mock_session.get.side_effect = aiohttp.ClientError("Persistent network error")
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_timeout_error(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_invalid_response_format(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_invalid_model_data(self, discovery_service, provider_config, mock_cache):
            ...
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                mock_response.json.return_value = {
                    "data": [
                        ...
                        {"id": "another-valid", "object": "model", "created": 1234567891, "owned_by": "test"}
                    ]
                }
                mock_session.get.return_value.__aenter__.return_value = mock_response
    ...
    @pytest.mark.asyncio
    async def test_model_discovery_empty_response(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_validate_model_success(self, discovery_service, provider_config, mock_model_data, mock_cache):
        ...
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                is_valid = await discovery_service.validate_model(provider_config, "gpt-4")

                assert is_valid is True

    @pytest.mark.asyncio
    async def test_validate_model_not_found(self, discovery_service, provider_config, mock_model_data, mock_cache):
        ...
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                is_valid = await discovery_service.validate_model(provider_config, "non-existent-model")

                assert is_valid is False

    @pytest.mark.asyncio
    async def test_validate_model_provider_error(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_get_model_info_success(self, discovery_service, provider_config, mock_model_data, mock_cache):
        ...
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                model_info = await discovery_service.get_model_info(provider_config, "gpt-4")
    ...
    @pytest.mark.asyncio
    async def test_get_model_info_not_found(self, discovery_service, provider_config, mock_model_data, mock_cache):
        ...
        with patch('src.core.model_discovery.get_unified_cache', return_value=mock_cache):
            with patch('aiohttp.ClientSession') as mock_session_class:
                ...
                model_info = await discovery_service.get_model_info(provider_config, "non-existent")

                assert model_info is None

    @pytest.mark.asyncio
    async def test_cache_invalidation(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_cache_invalidation_not_found(self, discovery_service, provider_config, mock_cache):
    ...
    @pytest.mark.asyncio
    async def test_provider_config_without_organization(self, discovery_service, mock_model_data, mock_cache):
...
class TestModelDiscoveryIntegrationScenarios:
    ...
    async def test_multiple_providers_different_responses(self, discovery_service):
        ...
        responses = [
            {
                "data": [
                    {"id": "model1", "object": "model", "created": 1234567890, "owned_by": "prov1"}
                ]
            },
            {
                "data": [
                    {"id": "model2", "object": "model", "created": 1234567891, "owned_by": "prov2"},
                    {"id": "model3", "object": "model", "created": 1234567892, "owned_by": "prov2"}
                ]
            }
        ]
    ...
    async def test_concurrent_model_discovery(self, discovery_service):
        ...
        async def mock_get(url, headers=None):
            ...
            response.json.return_value = {
                "data": [{"id": "model1", "object": "model", "created": 1234567890, "owned_by": "test"}]
            }
            return response
