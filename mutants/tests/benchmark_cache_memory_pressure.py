...
class CacheMemoryPressureBenchmark:
    ...
    def benchmark_model_cache_memory_pressure(self, max_size: int = 100) -> List[BenchmarkResult]:
        ...
        for pressure in pressure_levels:
            ...
            for i in range(total_accesses):
                ...
                cached = cache.get_models(f"provider-{key_idx}", f"https://provider-{key_idx}.com")
                if cached:
                    hits += 1
        ...
        for limit_mb in memory_limits:
            ...
            for i in range(200):  # More than enough to trigger limits
                try:
                    ...
                    if sum(e.size_bytes for e in cache._cache.values()) / (1024 * 1024) >= limit_mb:
                        break
                except Exception:
                    break
            ...
            results.append(BenchmarkResult(
                ...
                operations_per_second=entries_added / duration if duration > 0 else 0,
                eviction_count=len(cache._cache) - entries_added if len(cache._cache) < entries_added else 0,
                metadata={
    ...
    def benchmark_eviction_performance_impact(self) -> List[BenchmarkResult]:
        ...
        for max_size in cache_sizes:
            ...
            baseline_times = []
            for i in range(50):
                models = [ModelInfo(id=f"baseline-{i}", object="model", created=time.time(), owned_by="test")]
            ...
            for i in range(max_size * 3):  # Exceed capacity multiple times
                models = [ModelInfo(id=f"eviction-{i}", object="model", created=time.time(), owned_by="test")]
            ...
            results.append(BenchmarkResult(
                ...
                metadata={
                    ...
                    "performance_impact": (eviction_avg - baseline_avg) / baseline_avg if baseline_avg > 0 else 0
                }
            ))
    ...
    def benchmark_memory_recovery_patterns(self) -> List[BenchmarkResult]:
        ...
        for i in range(50):  # Exceed capacity significantly
            models = [ModelInfo(id=f"pressure-{i}", object="model", created=time.time(), owned_by="test")]
        ...
        for i in range(recovery_operations):
            models = [ModelInfo(id=f"recovery-{i}", object="model", created=time.time(), owned_by="test")]
