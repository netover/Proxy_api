...
class BenchmarkResult:
    ...
    def p95_response_time(self) -> float:
        if len(self.response_times) >= 20:  # Need enough samples for percentile
...
async def benchmark_connection_pooling():
    ...
    print(f"{'Metric':<25} {'V1 (Optimized)':<20} {'V2 (Advanced)':<20} {'Improvement':<15}")
    print("-" * 80)
    ...
    improvement = ((v1_median - v2_median) / v1_median * 100) if v1_median > 0 else 0
    ...
    # Error rates
    v1_error_rate = (v1_result.errors / num_requests * 100) if num_requests > 0 else 0
    v2_error_rate = (v2_result.errors / num_requests * 100) if num_requests > 0 else 0
    ...
    if "pool_info" in v1_result.metrics:
        ...
        print(f"V1 Pool Info: Total={pool_v1.get('total_connections', 0)}, "
              f"Available={pool_v1.get('available_connections', 0)}")

    if "pool_info" in v2_result.metrics:
        pool_v2 = v2_result.metrics["pool_info"]
        print(f"V2 Pool Info: Total={pool_v2.get('total_connections', 0)}, "
